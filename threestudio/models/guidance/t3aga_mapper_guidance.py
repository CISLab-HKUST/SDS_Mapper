...
@threestudio.register("t3aga-mapper-guidance")
class StableDiffusionT3AGAGuidance(BaseModule):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        # self.learnable_mapper1 = torch.nn.Parameter(torch.ones(1, 4, 64, 64).clone().to(self.device))
        # self.learnable_mapper1.requires_grad = True

        # self.learnable_mapper2 = torch.nn.Parameter(torch.zeros(1, 4, 64, 64).clone().to(self.device))
        # self.learnable_mapper2.requires_grad = True

        self.learnable_mapper = torch.nn.Parameter(torch.ones(1, 4, 64, 64).clone().to(self.device))
        self.learnable_mapper.requires_grad = True
    ...
    def compute_grad_mapper(
        ...
        # mapper1 = self.learnable_mapper1.repeat(B, 1, 1, 1)
        # mapper2 = self.learnable_mapper2.repeat(B, 1, 1, 1)

        mapper = self.learnable_mapper.repeat(B, 1, 1, 1)
        ...
        # noise_pred_pretrain = self.cfg.guidance_scale * (noise_pred_cond - noise_pred_uncon) + noise_pred_uncon - (noise_pred_uncon * mapper1.detach() + mapper2.detach())

        noise_pred_pretrain = self.cfg.guidance_scale * (noise_pred_cond - noise_pred_uncon) + noise_pred_uncon - noise_pred_uncon * mapper.detach()
    ...
    def train_mapper(
        ...
        # print(f'learnable mapper shape:{self.learnable_mapper.shape}')
        # mapper1 = self.learnable_mapper1.repeat(B, 1, 1, 1)
        # mapper2 = self.learnable_mapper2.repeat(B, 1, 1, 1)
        ...
        noise_pred_uncon = self.forward_unet(
            ...
            latents_noisy,
            torch.cat([t]),
            encoder_hidden_states=text_embeddings_vd[B:],
        )
        # return F.mse_loss((noise_pred_uncon * mapper1 + mapper2).float(), noise.detach().float(), reduction="mean")
        return F.mse_loss((noise_pred_uncon * mapper).float(), noise.detach().float(), reduction="mean")
    ...
    def forward(
        ...
        **kwargs,
        ...
        loss_mapper = self.train_mapper(latents, text_embeddings_vd, text_embeddings, camera_condition)
        grad = self.compute_grad_mapper(
